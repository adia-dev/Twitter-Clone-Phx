defmodule TwitterWeb.TwitterLive.Home do
  ...
  def mount(_params, session, socket) do
    ...
    socket =
      ...
      |> assign(:bio, user.bio)
      |> assign(:pfp_path, user.profile_picture_url)
      |> assign(:banner_url, user.banner_url)
      ...
      |> assign(:changeset, User.profile_changeset(%User{}, %{}))
  ...
  def handle_event("save", _params, socket) do
    consume_uploaded_entries(socket, :media, fn meta, entry ->
      filename = get_filename(entry, socket)
      dest = Path.join("priv/static/uploads/banners", filename)

      File.cp!(meta.path, dest)
      Routes.static_path(socket, "/uploads/banners/#{filename}")

      IO.inspect(
        Accounts.get_user_by_email(socket.assigns.email)
        |> Accounts.update_user(banner_url: "/uploads/banners/" <> filename)
      )
    end)
  ...
  def handle_event("save_pfp", _params, socket) do
    consume_uploaded_entries(socket, :media, fn meta, entry ->
      filename = get_filename(entry, socket)
      dest = Path.join("priv/static/uploads/pfps", filename)

      File.cp!(meta.path, dest)
      Routes.static_path(socket, "/uploads/pfps/#{filename}")

      IO.inspect(
        Accounts.get_user_by_email(socket.assigns.email)
        |> Accounts.update_user(profile_picture_url: "/uploads/pfps/" <> filename)
      )
    end)
  ...
  defp get_filename(entry, socket) do
    [ext | _rest] = MIME.extensions(entry.client_type)
    "#{socket.assigns.handle}.#{ext}"
  end

  defp file_extension(entry) do
    [ext | _rest] = MIME.extensions(entry.client_type)
    ext
  end
  ...
  def render(assigns) do
    ...
    <div class="h-screen w-screen flex grid-flow-col">
      <div id="menu" class="sticky h-screen border-r-[1px] border-gray-100
        ...
        <button
          class="font-semibold mt-7 text-white w-11/12 px-3 py-2 mx-auto rounded-3xl bg-[#1d9bf0]">Tweeter</button>
      ...
      <div id="header" class="sticky w-6/12">
        ...
        <div id="profile" class="w-full h-72 mb-6 object-cover relative">
          <%= if @banner_url == nil do%>
          <img class="w-full object-cover h-[200px] cursor-pointer brightness-[65%]"
            src="https://tweetbrander.com/wp-content/uploads/2013/01/twitter-header-post-640x360.png" alt="pfp">
          <% else %>
          <img class="w-full object-cover h-[200px] cursor-pointer brightness-[65%]" src={@banner_url} alt="pfp">
          <% end %>
          <div
            ...
            <img class="rounded-full h-full w-full object-cover"
              src="https://tweetbrander.com/wp-content/uploads/2013/01/twitter-header-post-640x360.png" alt="pfp">
            <% else %>
            <img class="rounded-full w-full h-full object-cover" src={@pfp_path} alt="">
            <% end %>

            <.form let={_f} for={@changeset} phx-change="validate_upload" class="w-full h-full">
              <div class="w-full h-full absolute top-0 left-0 z-10" phx-drop-target={@uploads.media.ref}>
                <%= live_file_input @uploads.media, class: "w-full h-full opacity-0" %>
              </div>

              <%= for entry <- @uploads.media.entries do%>
              <div class="">
                <%= live_img_preview entry, class: "w-full h-full object-cover absolute top-0 left-0" %>
                <h1><%= entry.progress %>%</h1>
              </div>
              <% end %>
            </.form>
          </div>
          <button phx-click="save_pfp"><i
              class="text-xl text-blue-500 cursor-pointer mt-24 fas fa-sync-alt"></i></button>
        ...
        <div class="py-3">
          ...
          <div id="feed" class="w-full ">
            <div class="py-1 px-5 flex flex-col cursor-pointer hover:bg-gray-100">
              <div class="ml-7 mb-1 flex items-center text-[#536471]">
                <svg height="14" fill="#536471" viewBox="0 0 24 24" aria-hidden="true">
                  <g>
                    <path
                ...
                <p class="ml-2 text-xs">Vous avez retweeté</p>
              </div>
              <div class="flex items-start">
                <img alt="" draggable="true"
                  src="https://pbs.twimg.com/profile_images/930865774461366272/0ihUr3bR_x96.jpg"
                  class="rounded-full h-12 mr-4">
                <div class="text-sm font-light">
                  <div class="flex items-center">
                    <h3 class="font-semibold text-sm mr-1">Pierrick Picaut</h3>
                    <p class="text-xs text-gray-400">@pierrick_picaut • 13 août</p>
                  </div>
                  <p>🥳🔥The time is now, Learn <span
                      class="text-blue-500 font-bold cursor-pointer hover:underline">#animation</span> in Blender with
                    the
                    most
                    extensive course released
                    to date 🔥🥳<br>
                    👉<span
                      class="text-blue-500 font-semibold cursor-pointer underline">https://p2design.gumroad.com/l/Alive_Blender_animation_course</span>👈<br>
                    25% OFF with the code : earlybird<br>
                    (till the 19th of august)<br>
                    presentation video : <span
                      class="text-blue-500 font-semibold cursor-pointer underline">https://youtu.be/K6lYYgLKH04</span>
                    <br><br>
                    🙏 Please RT for support! 🥰

                    <br><br>

                    <span class="text-blue-500 font-bold cursor-pointer hover:underline">#b3d</span>
                    <span class="text-blue-500 font-bold cursor-pointer hover:underline">#course</span>

                    <video autoplay loop muted preload="auto" class="rounded-3xl my-3"
                      src="https://video.twimg.com/ext_tw_video/1426172216572616704/pu/vid/1280x720/5hTqn_y2AE4ffWnZ.mp4?tag=12">
                      Video</video>
                  </p>
                </div>
              </div>
